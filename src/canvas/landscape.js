...
export default function imagegenerator(lines, center) {
  ...
  return {
    ...
    getImage: function getImage() {
      ...
      var textTool = createTextTool(context);

      const items = this.lines.map(function(item, index, array) {
        return {
          size: Math.ceil(item.height * ppmm),
          start: index !== 0 ? Math.ceil(array[index-1].height * ppmm) + 20: 0,
          text: item.value,
          font: item.font,
          color: item.color
        };
